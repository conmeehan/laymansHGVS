...
This is taken from mutalyzer (https://github.com/mutalyzer/mutalyzer)
...
class Grammar():
    ...
    GeneName = Word(str(alphanums) + '-', min=1)
    ...
    AccNoStem = NotAny('LRG_') + Combine(Word(str(alphas) + '_') + Number)
    ...
    AccNo = NotAny('LRG_') + \
            Combine(Word(str(alphas) + '_') + Number)('RefSeqAcc') + \
            Optional(Version)

    # BNF: UD -> `UD_' [a-Z]+ (`_' Number)+
    UD = Combine('UD_' + Word(str(alphas)) + OneOrMore('_' + Number))('RefSeqAcc')
